...
$email = $password = $confirm_password = $fname =$lname = $mobile=$employeeid="";
$email_err = $password_err = $confirm_password_err =$fname_err = $lname_err = $mobile_err=$employeeid_err= "";
...
//validate lastname
 ...
 //Validate mobile
  ...
  if(empty(trim($_POST["employeeid"]))){
        $employeeid_err = "name is req";
    } else{$employeeid = trim($_POST["employeeid"]);}
    // Check input errors before inserting in database
    if(empty($fname_err)&&empty($lname_err)&&empty($employeeid_err)  && empty($password_err) && empty($confirm_password_err)&&empty($mobile_err)&&empty($email_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO employee (fname,lname,mobile,email,employeeid, password) VALUES (?, ? ,?,?,?,?)";
         
        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "ssssss",$param_fname,$param_lname,$param_mobile, $param_email,$param_employeeid, $param_password);
      ...
      <div class="site-section-cover overlay" data-stellar-background-ratio="0.5" style="background-color:#2F41C2">
        <div class="container">
          <div class="row align-items-center justify-content-center text-center">
            <div class="col-md-7">	 
		  <form name="form_login" method="post" action="AddEmployee.php" role="form">
	        ...
	        <input type="text" name="lname"  id="lname" class="form-control input-lg" placeholder="Surame">
               ...
               <div class="col-md-4 form-group">
                 <input type="text" name="employeeid"  id="employeeid" class="form-control input-lg" placeholder="Employee ID">  
               </div>
